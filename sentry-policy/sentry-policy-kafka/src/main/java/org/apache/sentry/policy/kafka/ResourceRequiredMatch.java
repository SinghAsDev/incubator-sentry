...
public class ResourceRequiredMatch implements PrivilegeValidator {
  
  public ResourceRequiredMatch() {
  }
  ...
  public void validate(PrivilegeValidatorContext context)
    ...
    for (KafkaAuthorizable authorizable : authorizables) {
      ...
      if (hostnameMatched && resourceMatched)
        break;
    }
    if (!hostnameMatched) {
    ...
    /*if (!resourceMatched) {
      String msg = "[cluster|consumer_group|topic]=[name] in " + context.getPrivilege()
          + " is required.";
      throw new ConfigurationException(msg);
    }*/
  }
