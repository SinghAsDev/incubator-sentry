...
public class SentryKafkaAuthorizer implements Authorizer {
  ...
  private static Logger LOG = LoggerFactory.getLogger(SentryKafkaAuthorizer.class);
  private final String COMPONENT_NAME = KafkaAuthConf.AuthzConfVars.getDefault(KafkaAuthConf.KAFKA_SERVICE_INSTANCE_NAME);

  private KafkaAuthBinding binding;
  private KafkaAuthConf kafkaAuthConf;
  private String kafkaServiceInstanceName = COMPONENT_NAME;
  ...
  private TSentryPrivilege toTSentryPrivilege(Acl acl, Resource resource) {
    ...
    TSentryPrivilege tSentryPrivilege = new TSentryPrivilege(COMPONENT_NAME, kafkaServiceInstanceName, tAuthorizables, acl.operation().name());
      ...
      public Void run(SentryGenericServiceClient client) throws Exception {
        for (String role: roles) {
          tSentryPrivileges.addAll(client.listPrivilegesByRoleName(
              loginUser.getShortUserName(), role, COMPONENT_NAME, kafkaServiceInstanceName));
        }
        return null;
      }
    });
  ...
  private Map<Resource, Set<Acl>> getAclsForRoles(final List<String> roles) {
    ...
    return scala.collection.JavaConverters.mapAsScalaMapConverter(resourceAclsMap).asScala().toMap(Predef.<Tuple2<Resource, Set<Acl>>>conforms());
  }
